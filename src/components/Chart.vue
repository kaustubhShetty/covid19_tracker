<template>
  <div>
    <h1 id='app'>State/UT:{{this.$store.state.stateName}}</h1>
    <line-chart :key="componentKey" :data="chartData" />
    <div>{{this.DictionaryOfDatesAndConfirmedProperty}}</div>
  </div>
</template>
...
export default {
  ...
  mounted(){
   ...
   methods: {
    ...
    getConfirmedCasesforSpecificState(statename){
      for(let i=0; i<this.states_daily_list.length; i = i+3){
        this.confirmed_cases_list.push(parseInt(this.states_daily_list[i][statename]));
      }
    ...
    getRecoveredCasesforSpecificState(statename){
      for(let i=1; i<this.states_daily_list.length; i = i+3){
        ...
        this.recovered_cases_list.push(parseInt(this.states_daily_list[i][statename]));
      }
    ...
    DictionaryOfDatesAndConfirmed(){
      this.getConfirmedCasesforSpecificState(this.$store.state.stateName);
      for(let i=0;i<this.dates.length;i++){
       this.DictionaryOfDatesAndConfirmedProperty[this.dates[i]]=this.confirmed_cases_list[i];
      }
    ...
    DictionaryOfDatesAndRecovered(){
      this.getRecoveredCasesforSpecificState(this.$store.state.stateName);
      for(let i=0;i<this.dates.length;i++){
       this.DictionaryOfDatesAndRecoveredProperty[this.dates[i]]=this.recovered_cases_list[i];
      }
  ...
  // watch:{
  //   DictionaryOfDatesAndConfirmedProperty: this.DictionaryOfDatesAndConfirmed(),
  //   DictionaryOfDatesAndRecoveredProperty: this.DictionaryOfDatesAndRecovered(),
  //  }
};
</script>
